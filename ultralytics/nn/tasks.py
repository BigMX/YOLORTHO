...
from ultralytics.nn.modules import (C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x, Classify,
                                    ...
                                    GhostBottleneck, GhostConv, Segment, ICustom, IDetect)
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Segment, ICustom, IDetect)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, (ICustom, IDetect)):
            s = 256  # 2x min stride
            m.inplace = self.inplace
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Segment, ICustom, IDetect):
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        elif m in {ICustom, IDetect}:
            args.append([ch[x] for x in f])
            args[0] = d['config']

            pass
            # TODO: Theoretically should look into config to make channel divisible
            # but no for now.
            # args[2] = make_divisible(args[2] * gw, 8)
        else:
            c2 = ch[f]
