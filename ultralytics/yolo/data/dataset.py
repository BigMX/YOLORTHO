...
from .utils import HELP_URL, LOCAL_RANK, get_hash, img2label_paths, img2attr_paths, verify_custom_image_label, verify_image_label
...
class CustomDataset(BaseDataset):
    ...
    def cache_labels(self, path=Path('./labels.cache')):
        ...
        total = len(self.im_files)
        with ThreadPool(NUM_THREADS) as pool:
            results = pool.imap(func=verify_custom_image_label,
                                iterable=zip(self.im_files, self.label_files, self.attribute_files, repeat(self.prefix),
                                             repeat(self.use_keypoints), repeat(len(self.names))))
            pbar = tqdm(results, desc=desc, total=total, bar_format=TQDM_BAR_FORMAT)
            for im_file, lb, shape, segments, keypoint, nm_f, nf_f, ne_f, nc_f, msg, at in pbar:
                nm += nm_f
                nf += nf_f
                ne += ne_f
                nc += nc_f
        ...
        if nf == 0:
            LOGGER.warning(f'{self.prefix}WARNING ⚠️ No labels found in {path}. {HELP_URL}')
        x['hash'] = get_hash(self.label_files + self.im_files + self.attribute_files)
        x['results'] = nf, nm, ne, nc, len(self.im_files)
        x['msgs'] = msgs  # warnings
        x['version'] = self.cache_version  # cache version
        if is_dir_writeable(path.parent):
    ...
    def get_labels(self):
        if self.label_files is None:
            self.label_files = img2label_paths(self.im_files)
            self.attribute_files = img2attr_paths(self.im_files)
        cache_path = Path(self.label_files[0]).parent.with_suffix('.cache')
        try:
            cache, exists = np.load(str(cache_path), allow_pickle=True).item(), True  # load dict
            assert cache['version'] == self.cache_version  # matches current version
